...
`@deepseek-ai/dsh-subprocess-local/managed-process` 子路径公开普通的 detached 进程树原语，无需挂载 Cordis Service Provider，也不会加载终端实现。宿主应用可以要求隐藏 Windows 子进程窗口，并保留本地同步宿主退出回退；stdio、环境、宽限期和其他全部进程策略仍须在 spawn spec 中显式给出。
