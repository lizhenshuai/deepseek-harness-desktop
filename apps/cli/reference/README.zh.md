...
进程关闭时，插件树最多有 5 秒完成 dispose。首次收到 `SIGINT` 或 `SIGTERM` 时会开始优雅排空：`SIGTERM` 是监督进程发出的常规停止请求，在所有运行模式下都以 0 退出；`SIGINT` 则报告 130。第二次收到信号时会立即强制退出。如果一次性运行在正常结束时已经卡在 dispose 阶段，第一次按下 `Ctrl+C` 就会直接升级为强制退出，而不会被忽略。仅属于启动器的 `--supervised-stdin` flag 接受精确的 `shutdown\n` 帧，或把空的父进程管道 EOF 视为常规退出请求；畸形或不完整输入会先请求同一套 dispose，再以非零状态退出。该 flag 必须位于应用参数之前，供 Windows 桌面 shell 等进程所有者使用。
